...
import { IotEdgedAuthenticationProvider } from './iotedged_authentication_provider';
...
export class InternalClient extends EventEmitter {
  ...
  static validateEnvironment(): void {
    ...
    keys.forEach((key) => {
      if (!process.env[key]) {
        throw new errors.PreconditionFailedError(`Envrionment variable ${key} was not provided.`);
      }
    });

    // we only support sas token auth scheme at this time
    if (process.env.IOTEDGE_AUTHSCHEME !== 'SasToken') {
      throw new errors.PreconditionFailedError(`Authentication scheme ${process.env.IOTEDGE_AUTHSCHEME} is not a supported scheme.`);
    }
  }
  ...
  static fromEnvironment(transportCtor: any, clientCtor: any): any {
    ...
    if (connectionString) {
      return InternalClient.fromConnectionString(connectionString, transportCtor, clientCtor);
    }
    ...
    const authenticationProvider = new IotEdgedAuthenticationProvider(authConfig);
