...
export class AmqpTwinReceiver extends EventEmitter {
  ...
  constructor(config: ClientConfig, client: any) {
    ...
    this._fsm = new machina.Fsm({
      ...
      states: {
        'disconnected': {
          _onEnter: () => {
            let headEvent: FsmEvent;
            while (headEvent = this._removeHeadOfEventQueue()) {
              if (headEvent.event === 'sendTwinRequest') {
                if (headEvent.arguments[_doneArgumentIndex]) {
                  headEvent.arguments[_doneArgumentIndex](this._eventQueueError || new Error('Link Detached'));
                }
              }
            }
            this._eventQueueError = null;
  ...
  private _handleError(err: Error): void {
    this._fsm.handle('handleLinkDetach', err);
  }
  ...
  private _appendEventQueue(): void {
    this._eventQueue.push({
      event: this._fsm.currentActionArgs[0].inputType,
      arguments: this._fsm.currentActionArgs.splice(1, this._fsm.currentActionArgs.length - 1)
    });
  }

  private _removeHeadOfEventQueue(): FsmEvent {
    if (this._eventQueue.length > 0) {
      const head: FsmEvent = this._eventQueue[0];
      this._eventQueue = this._eventQueue.splice(1, this._eventQueue.length - 1);
      return head;
    } else {
      return null;
    }
  ...
  private _handleHeadOfEventQueue(): void {
    let head: FsmEvent = this._removeHeadOfEventQueue();
    if (head) {
      this._fsm.handle('actual_' + head.event, ...head.arguments);
    }
  }

  private _AppendOrHandleEvent(): void {
    if (this._eventQueue.length === 0) {
      this._fsm.handle('actual_' + this._fsm.currentActionArgs[0].inputType, ...this._fsm.currentActionArgs.splice(1, this._fsm.currentActionArgs.length - 1));
    } else {
      this._appendEventQueue();
    }
  }
